...
class DitaFileSet(DitaLinkCheckBase):
    ...
    def __init__(self, theDir, procDir=True, thePatterns=None, recursive=False, testExt=False):
    ...
    def writeErrors(self, isGeneric, theFilter, theStream=sys.stdout):
        ...
        fileS.sort()
        for aFile in fileS:
            self._fileMap[aFile].writeErrors(isGeneric, theFilter, theStream)
    ...
    def allErrStrings(self, isGeneric, theFilter):
        ...
        fileS = self._fileMap.keys()
        fileS.sort()
        for aFileObj in self._fileMap.values():
            for anErr in aFileObj.errStrings(isGeneric, theFilter): 
                retSet.add(anErr)
        retList = list(retSet)
        retList.sort()
        return retList
    ...
    def _addDitaFileObj(self, theDitaFileObj):
        if self._fileMap.has_key(theDitaFileObj.identity):
            ...
            self._fileMap[theDitaFileObj.identity] = theDitaFileObj
    ...
    def _initRootIdToFilePathMap(self):
        ...
        myDupeIdFiles = {}
        for fPath, fObj in self._fileMap.items():
    ...
    def _checkDupeIdS(self):
        ...
        seenIdMap = {}
        for f, o  in self._fileMap.items():
            for anId in o.idS:
    ...
    def _retMapAdjList(self):
        ...
        adjList = {}
        for fObj in self._fileMap.values():
    ...
    def _checkLonelyMaps(self):
        ...
        mapPathSet = set()
        pathSetRemain = set()
        for f, o in self._fileMap.iteritems():
            if o.isMap:
                mapPathSet.add(f)
                pathSetRemain.add(f)
        for aPath in mapPathSet:
            myMapObj = self._fileMap[aPath]
    ...
    def _checkLonelyTopics(self):
        ...
        for aMapPath in mapPathSet:
            myMapObj = self._fileMap[aMapPath]
            for r in myMapObj.refS:
    ...
    def _checkRefArcs(self):
        ...
        for fPath, fObjSrc in self._fileMap.iteritems():
            for rObjSrc in fObjSrc.refS:
                if rObjSrc.scheme:
                    # Decide wether to test and external URL
                    if self._testExt:
                        rObjSrc.checkUrl()
                else:
                    ...
                    try:
                        ...
                        self._checkRefArcElemName(fObjSrc, rObjSrc, fObjTgt, fr)
    ...
    def debugDump(self, s=sys.stdout, prefix=''):
        ...
        fileS.sort()
        for f in fileS:
            self._fileMap[f].debugDump(s, prefix)
...
def retMpDitaFileSetObj(theDir, thePatterns, recursive, numJobs):
    assert(os.path.isdir(theDir))
    assert(numJobs >= 0)
    retObj = DitaFileSet(theDir, procDir=False)
    myNumJobs = numJobs
    if numJobs == 0:
